...
const Hub: React.FC = () => {
    ...
    const renderChevron = (isOpen: boolean) => (
        <svg
            ...
            className={`hb-chevron-icon ${isOpen ? 'hb-open' : ''}`}
        >
    ...
    const renderAddCharacterCard = () => (
        ...
        >
            <div className="hb-add-card-content">
                ...
                <span className="hb-add-card-label">Add new Character</span>
            </div>
        </div>
    );
    ...
    const renderAddCampaignCard = () => (
        ...
        >
            <div className="hb-add-card-content">
                ...
                <span className="hb-add-card-label">Add new Campaign</span>
            </div>
        </div>
    );
    ...
    const renderAddItemCard = () => (
        ...
        >
            <div className="hb-add-card-content">
                ...
                <span className="hb-add-card-label">Add new Item</span>
            </div>
        </div>
    );
    ...
    const renderAddSpellCard = () => (
        ...
        >
            <div className="hb-add-card-content">
                ...
                <span className="hb-add-card-label">Add new Spell</span>
            </div>
        </div>
    );
    ...
    const renderDeathSaves = (character: any) => {
        ...
        return (
            <div className="hb-death-saves-indicator">
                <span className="hb-death-saves-label">Death Saves</span>
                <div className="hb-death-saves-dots">
                    {[0, 1, 2].map(i => (
                        <span key={`s-${i}`} className={`hb-dot ${i < successes ? 'hb-success' : 'hb-empty'}`} />
                    ))}
                    <span className="hb-separator">/</span>
                    {[0, 1, 2].map(i => (
                        <span key={`f-${i}`} className={`hb-dot ${i < failures ? 'hb-failure' : 'hb-empty'}`} />
                    ))}
                </div>
            </div>
        );
        ...
        <div className="hb-page">
            <div className="hb-header">
                <div className="hb-header-content">
                    <div className="hb-header-left">
                        <span className="hb-title">Hub</span>
                        <div className="hb-subtitle">Select a character or explore campaigns</div>
                    </div>
                    <div className="hb-header-icon" onClick={() => {}} style={{ cursor: 'pointer' }}>
            ...
            <div className="hb-content">
                ...
                <div className="hb-section">
                    <div className="hb-section-header" onClick={toggleCharacters} style={{ cursor: 'pointer' }}>
                        <span className="hb-section-title">Your Characters</span>
                        <div className="hb-section-header-right">
                            <Link to="/dashboard" className="hb-view-all-link" onClick={(e) => e.stopPropagation()}>View all</Link>
                            {renderChevron(isCharactersOpen)}
                        </div>
                    </div>
                    {isCharactersOpen && (
                        <div className="hb-character-grid">
                            {filteredCharacters.slice(0, 5).map((char) => {
                                const needsDeathSave = char.hp === 0 && char.status !== 'dead' && !char.isStable;
                                return (
                                    <Link to={`/characters/${char.id}`} key={char.id} className="hb-character-card-link" style={{ textDecoration: 'none' }}>
                                        <div className={`hb-character-card ${char.status !== 'active' ? 'hb-inactive' : ''} ${needsDeathSave ? 'hb-needs-death-save' : ''}`}>
                                            <div className="hb-character-info">
                                                <div className="hb-character-name">{char.name}</div>
                                                <div className="hb-character-class">{char.classes.join(' / ')} • Level {char.level}</div>
                                                <div className="hb-character-created">Created: {char.created || 'N/A'}</div>
                                                <div className="hb-character-date">{getCharacterDateLabel(char.status, char.lastUsed || char.died || char.archived)}</div>
                                                {needsDeathSave && renderDeathSaves(char)}
                                            </div>
                                            <div className={`hb-character-status ${char.status || 'active'}`}>
                ...
                {/* Секция кампаний */}
                <div className="hb-section">
                    <div className="hb-section-header" onClick={toggleCampaigns} style={{ cursor: 'pointer' }}>
                        <span className="hb-section-title">Campaigns</span>
                        <div className="hb-section-header-right">
                            <Link to="/campaigns" className="hb-view-all-link" onClick={(e) => e.stopPropagation()}>View all</Link>
                ...
                {/* Секция предметов */}
                <div className="hb-section">
                    <div className="hb-section-header" onClick={toggleItems} style={{ cursor: 'pointer' }}>
                        <span className="hb-section-title">Items</span>
                        <div className="hb-section-header-right">
                            <Link to="/items" className="hb-view-all-link" onClick={(e) => e.stopPropagation()}>View all</Link>
                            {renderChevron(isItemsOpen)}
                        </div>
                    </div>
                    {isItemsOpen && (
                        <div className="hb-items-grid">
                            {filteredItems.slice(0, 5).map((item) => (
                                <Link to={`/item/${item.id}`} key={item.id} className="hb-item-card-link" style={{ textDecoration: 'none' }}>
                                    <div className="hb-item-card">
                                        <div className="hb-item-info">
                                            <div className="hb-item-name">{item.name}</div>
                                            <div className="hb-item-description">{truncateDescription(item.description)}</div>
                                            <div className="hb-item-details">
                                                {item.healing && (
                                                    <span className="hb-item-tag-healing">Healing</span>
                                                )}
                                                {item.charges && (
                                                    <span className="hb-item-charges">Uses: {item.charges.current}/{item.charges.max}</span>
                                                )}
                                                {item.diceRoll && (
                                                    <span className="hb-item-dice" style={{ color: item.healing ? elementColors.healing : undefined }}>
                                                        {item.diceRoll}
                                                    </span>
                                                )}
                                            </div>
                ...
                {/* Секция заклинаний */}
                <div className="hb-section">
                    <div className="hb-section-header" onClick={toggleSpells} style={{ cursor: 'pointer' }}>
                        <span className="hb-section-title">Spells</span>
                        <div className="hb-section-header-right">
                            <Link to="/spells" className="hb-view-all-link" onClick={(e) => e.stopPropagation()}>View all</Link>
                            {renderChevron(isSpellsOpen)}
                        </div>
                    </div>
                    {isSpellsOpen && (
                        <div className="hb-spells-grid">
                            {filteredSpells.slice(0, 5).map((spell) => (
                                <Link to={`/spell/${spell.id}`} key={spell.id} className="hb-spell-card-link" style={{ textDecoration: 'none' }}>
                                    <div className="hb-spell-card">
                                        <div className="hb-spell-info">
                                            <div className="hb-spell-name">{spell.name}</div>
                                            <div className="hb-spell-description">{truncateDescription(spell.description)}</div>
                                            <div className="hb-spell-details">
                                                <span className="hb-spell-level">
                                                    {spell.level === 0 ? 'Cantrip' : `Lv.${spell.level}`}
                                                </span>
                                                <span className="hb-spell-school">{spell.school}</span>
                                                {spell.element && (
                                                    <span className="hb-spell-element" style={{ color: elementColors[spell.element] || '#6b7280' }}>
                                                        {spell.element}
                                                    </span>
                                                )}
                                                {spell.diceRoll && spell.element && elementColors[spell.element] && (
                                                    <span className="hb-spell-dice" style={{ color: elementColors[spell.element] }}>
                                                        {spell.diceRoll}
                                                    </span>
                                                )}
                                                {spell.diceRoll && !spell.element && (
                                                    <span className="hb-spell-dice">{spell.diceRoll}</span>
                                                )}
                                            </div>
                                        </div>
                                    </div>
